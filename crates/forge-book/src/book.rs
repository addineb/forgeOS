...
/// iteration order is deterministic. The bid map's max key and the ask map's
/// min key are the best quotes.
...
impl OrderBook {
    ...
    /// Two folds of the same event stream produce the same hash.
    #[must_use]
...
mod tests {
    ...
    fn remove_deletes_level() {
        let mut b = OrderBook::new();
        b.apply(&delta(Side::Bid, 100, 5, 1)).unwrap();
        b.apply(&delta(Side::Bid, 100, 0, 2)).unwrap(); // remove
    ...
    fn crossing_bid_trims_asks() {
        ...
        // a bid at 101 consumes the ask at 101 (and only it)
