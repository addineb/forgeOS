...
/// that just arrived and the book folded up to and including it. There is no
/// API to read any future event - no-lookahead is enforced structurally.
pub struct Ctx<'a> {
...
/// An order a strategy wants to send. Matching/fills are Phase 2; here it only
/// flows through the outbound latency queue.
...
/// A trading strategy: pure reaction to `(event, book, now)`.
pub trait Strategy {
