...
use clap::Parser;
use forge_anomaly::{
    load_volume_bars, load_volume_bars_with_fwd, AnomalyEngine, AnomalyKind,
    ...
    if args.verbose {
        writeln!(
            ...
            "bar_index,ts,row_type,kind,direction,signal_type,z_score,confidence,\
             maha_dist,description,mid_price"
        )?;
    } else {
        writeln!(
            out,
            "bar_index,ts,direction,signal_type,confidence,maha_dist,\
        ...
        if args.verbose {
            for ev in &output.anomalies {
                writeln!(
                    out,
                    "{},{},anomaly,{},{},,{:.4},{:.4},{:.4},,{}",
                    ev.bar_index, ev.ts, kind_str(ev.kind), dir_str(ev.direction),
                    ev.z_score, ev.confidence, output.mahalanobis_dist, bar.mid_price,
                )?;
            }
        }
        if let Some(sig) = &output.signal {
            ...
            if args.verbose {
                writeln!(
                    out,
                    "{},{},signal,,{},{},{:.4},{:.4},\"{}\",{}",
                    sig.bar_index, sig.ts, dir_str(sig.direction),
                    sig.signal_type, sig.confidence, sig.mahalanobis_dist,
                    sig.description.replace('"', "'"), bar.mid_price,
                )?;
            } else {
                writeln!(
                    out,
                    ...
                    sig.bar_index, sig.ts, dir_str(sig.direction),
                    sig.signal_type, sig.confidence, sig.mahalanobis_dist,
                    sig.expected_move_bps, sig.hold_bars, sig.pattern_count,
...
fn run_backtest(args: &Args) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (maha_c, _) = forge_anomaly::calibrate_expected_move(&eval_rows);
    eprintln!("  calibration: suggested maha_coeff={:.3}", maha_c);

    let backtest_path = PathBuf::from(
        args.output.with_extension("").to_string_lossy().to_string() + "_backtest.csv",
    );
    let mut out = File::create(&backtest_path)?;
    writeln!(
        out,
        "bar_index,ts,direction,signal_type,confidence,fwd_ret_15m_bps,fwd_ret_1h_bps,\
         fwd_ret_4h_bps,signed_ret_1h_bps,expected_move_bps"
    )?;
    for r in &eval_rows {
        ...
        writeln!(
            out,
            "{},{},{},{},{:.4},{:.4},{:.4},{:.4},{:.4},{:.4}",
            r.bar_index, r.ts, dir_str(r.direction),
            r.signal_type, r.confidence,
            r.fwd.fwd_ret_15m_bps, r.fwd.fwd_ret_1h_bps, r.fwd.fwd_ret_4h_bps,
            signed_1h, r.expected_move_bps,
        )?;
    }
